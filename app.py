...
# 3. 메인 앱
def smart_purchase_manager_neulbarun_v45():
    ...
    # [교정] 에러 방지를 위해 세션 상태에서만 관리하는 변환 로직
    def smart_unit_converter(key):
        val = st.session_state[key]
        if 0 < val <= 5000: # 5000(5천만원) 이하 숫자가 들어오면 만원 단위로 판단
    ...
    with left_col:
        ...
        # [수정] 충돌 방지를 위해 value 제거하고 session_state를 통해 1:1 변환
        # step=1로 하여 소수점 발생 원천 봉쇄
        in_dent = st.number_input("판금/도색", step=1, key='in_dent', on_change=smart_unit_converter, args=('in_dent',), format="%d")
        in_wheel = st.number_input("휠/타이어", step=1, key='in_wheel', on_change=smart_unit_converter, args=('in_wheel',), format="%d")
        in_etc = st.number_input("기타비용", step=1, key='in_etc', on_change=smart_unit_converter, args=('in_etc',), format="%d")
    ...
    with st.expander("🧾 상세 내역 및 복사 (펼치기)", expanded=True):
        ...
        with d_col1:
            ...
            <table class='detail-table'>
                ...
                <tr><td>상품화비용</td><td align='right'>{int(cost_dent_vat + cost_wheel_vat + cost_etc_vat):,} 원</td></tr>
            </table>
            """, unsafe_allow_html=True)
        with d_col2:
            st.caption("▼ 복사 전용 텍스트")
            copy_text = f"판매가: {int(sales_price):,}원\n매입가: {int(my_bid):,}원\n수익률: {real_margin_rate:.2f}%\n순수익: {int(real_income):,}원\n-----------------\n판금도색: {int(cost_dent_vat):,}원\n휠타이어: {int(cost_wheel_vat):,}원\n기타비용: {int(cost_etc_vat):,}원\n매입등록: {int(res_reg):,}원\n낙찰수수: {int(res_fee):,}원"
            st.code(copy_text, language="text")

if __name__ == "__main__":
    smart_purchase_manager_neulbarun_v45()
